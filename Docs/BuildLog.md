...
- Added a camera/photo-library surface, EXIF-stripping image normalization, in-thread analysis review, correction/recalculation, explicit confirmation, later refinement, and destructive-delete confirmation. Photo bytes remain transient until confirmation; a confirmed meal retains the prepared photo in protected local storage so the same visual appears in the thread and atlas.
