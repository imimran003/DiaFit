...
3. Selecting the photo immediately starts analysis; a description and a second “upload” action are not required. The app first tries the private `VNClassifyImageRequest` fast path, collects candidate labels at 0.20 or above, and admits only labels that map to the canonical food catalog. Candidate confidence is propagated into the meal result; a local result can bypass the backend only at medium confidence (0.80) or better. `PhotoAnalysisCompletenessEvaluator` also checks identity, serving conversion, core nutrition, provenance and validation. Any low-confidence or incomplete result automatically escalates to the configured schema-constrained backend vision parser.
...
- The whole-image on-device classifier is a privacy-preserving candidate generator, not a portion-measurement system. A 0.20 collection threshold retains potentially useful catalog labels, while the separate 0.80 completeness threshold prevents a weak generic label from being treated as final recognition. Deterministic tests verify that a low-confidence “salad” label escalates to structured photo interpretation while a high-confidence banana remains on the fast path.
