...
- To keep private photo recognition working while the Mac is asleep, deploy the backend with the checked-in [`render.yaml`](render.yaml). Follow [`Docs/HostedBackend.md`](Docs/HostedBackend.md); only the hosted HTTPS URL and app-to-backend token are configured in Xcode, while the Gemini key remains a host secret.
- Food art uses consistent local studio cutouts only for known sample meals. Analysed free-text meals create a quantity-sensitive visual request and use a deterministic component composition until a verified image service is configured; an unrelated food is never used as a fallback.

Further decision records live in [Docs/Intent.md](Docs/Intent.md), [Docs/IndianFoodAnalysisPlan.md](Docs/IndianFoodAnalysisPlan.md), [Docs/PhotoAnalysisArchitecture.md](Docs/PhotoAnalysisArchitecture.md), [Docs/CorrectnessAudit.md](Docs/CorrectnessAudit.md), and [Docs/FoodUnderstandingPipeline.md](Docs/FoodUnderstandingPipeline.md).
